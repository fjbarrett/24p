...
struct SimplifiedMovie: Codable, Identifiable, Hashable {
    ...
    /// Swaps the w185 thumbnail for a larger size for phone display.
...
struct SearchResultItem: Codable, Hashable {
    ...
    var movie: SimplifiedMovie? {
        guard resultType == "movie", let tmdbId, let title else { return nil }
        return SimplifiedMovie(
            tmdbId: tmdbId, title: title, mediaType: mediaType, overview: overview,
            releaseYear: releaseYear, rating: rating, imdbRating: imdbRating,
            posterUrl: posterUrl, backdropUrl: backdropUrl, runtime: runtime,
            genres: genres, tagline: tagline, imdbId: imdbId, director: director, cast: cast
        )
    }
}
...
struct StreamingCatalogMovie: Codable, Identifiable, Hashable {
    ...
    var asSimplifiedMovie: SimplifiedMovie {
        SimplifiedMovie(
            tmdbId: tmdbId, title: title, mediaType: mediaType, overview: overview,
            releaseYear: releaseYear, rating: justWatchRating, imdbRating: imdbRating,
            posterUrl: posterUrl, backdropUrl: backdropUrls.first, runtime: nil,
            genres: nil, tagline: nil, imdbId: imdbId, director: nil, cast: nil
        )
    }
}
...
struct SavedList: Codable, Identifiable, Hashable {
    ...
    var isEditable: Bool { canEdit ?? false }
...
struct SearchResponse: Codable {
    ...
    var results: [SimplifiedMovie] { combined.compactMap(\.movie) }
}
